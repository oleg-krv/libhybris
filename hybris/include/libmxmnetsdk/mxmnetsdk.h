...
{
    unsigned int iMaxEncodePower;		///< 支持的最大编码能力 Supported maximum coding capacity
    // Each channel whether the resolution needs to be synchronized 0 - do not sync 1 - Synchronization
    int iChannelMaxSetSync;		///< 每个通道分辨率是否需要同步 0-不同步, 1 -同步
    unsigned int nMaxPowerPerChannel[NET_MAX_CHANNUM];		///< 每个通道支持的最高编码能力 Each channel supports a maximum coding ability
    unsigned int ImageSizePerChannel[NET_MAX_CHANNUM];		///< 每个通道支持的图像分辨率 Each channel supports image resolution
    unsigned int ExImageSizePerChannel[NET_MAX_CHANNUM];		///< 每个通道支持的辅码流图像分辨率 Each channel supports secondary stream image resolution
    SDK_EncodeInfo vEncodeInfo[SDK_CHL_FUNCTION_NUM];	///< 编码信息,暂时最大就4中码流 Encoding information on the temporary maximum 4 Stream
    SDK_EncodeInfo vCombEncInfo[SDK_CHL_FUNCTION_NUM];	///< 组合编码信息,暂时最大就4中码流 Combination of encoding information on the temporary maximum 4 Stream
    int	iMaxBps;				///< 最高码流Kbps Highest stream Kbps
    unsigned int ExImageSizePerChannelEx[NET_MAX_CHANNUM][SDK_CAPTURE_SIZE_EXT_V3_NR];	///< 每个通道支持的辅码流图像分辨率 Each channel supports secondary stream image resolution
...
typedef struct
{
    int		        nPacketType;		// MEDIA_PACK_TYPE //包类型,见
    char*	        pPacketBuffer;		// 缓存区地址
    unsigned int	dwPacketSize;		// 包的大小

    // Absolute time scale //绝对时标
    int		        nYear;				// 时标:年
    int		        nMonth;				// 时标:月
    int		        nDay;				// 时标:日
    int		        nHour;				// 时标:时
    int		        nMinute;			// 时标:分
    int		        nSecond;			// 时标:秒
    unsigned int    dwTimeStamp;		//Low relative timing in milliseconds // 相对时标低位，单位为毫秒
    unsigned int    dwTimeStampHigh;    //When the relative elevation digit milliseconds //相对时标高位，单位为毫秒
    unsigned int    dwFrameNum;         //Frame Number //帧序号
    unsigned int    dwFrameRate;        //Frame rate //帧率
